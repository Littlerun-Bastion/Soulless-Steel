...
#Return highest-priority valid goal
func get_best_goal(goals, agent):
	var highest_priority
	for goal in goals:
		if goal.is_valid(agent) and (highest_priority == null or goal.priority() > highest_priority.priority()):
			highest_priority = goal

	return highest_priority


func get_plan(goal: Goal, actions, agent, blackboard) -> Array:
  ...
  # In case it doesn't find a valid path, it will return false.
	if build_plans(root, actions, blackboard.duplicate()):
		var plans = transform_tree_into_array(root, blackboard)
...
func build_plans(step, actions, blackboard):
  ...
  # more actions
	...
	# desired state
		...
		if should_use_action:
			...
			if desired_state.is_empty() or build_plans(s, actions, blackboard.duplicate()):
...
#
func transform_tree_into_array(p, blackboard):
	var plans = []
	...
	for c in p.children:
		for child_plan in transform_tree_into_array(c, blackboard):
