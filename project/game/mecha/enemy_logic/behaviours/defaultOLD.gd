...
#Essential variables
var nodes = ["roaming", "ambushing", "locking", "barraging", "cooling", "defending"]
...
func barraging_to_defending(enemy):
	var priority = 0
	if enemy.under_fire_timer > 0:
		priority = 15
	return priority

func defending_to_barraging(enemy):
	var priority = 0
	if enemy.under_fire_timer <= 0:
		priority = 15
	return priority
