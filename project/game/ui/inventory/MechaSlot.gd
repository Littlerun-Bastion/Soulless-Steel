...
func _input(event):
	if Input.is_action_just_pressed("left_click") and get_global_rect().has_point(get_global_mouse_position()) and part_data and part_data.part_id != "Null" and visible:
		equip_item(false)
	if Input.is_action_just_released("left_click") and get_global_rect().has_point(get_global_mouse_position()) and visible and ItemManager.item_held:
		equip_item(true)
...
func equip_item(equipping):
	var _item
	var _type = type
	if equipping and ItemManager.item_held and ItemManager.item_held.part_type in type:	
		_item = ItemManager.item_held.item_id
	elif not equipping:
		unequip_item(_type)
		return
	elif ItemManager.item_held and not ItemManager.item_held.part_type in type:
		return
